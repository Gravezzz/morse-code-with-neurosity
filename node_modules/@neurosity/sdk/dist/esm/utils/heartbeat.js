...
export function heartbeatAwareStatus(status$) {
    ...
    }).pipe(withLatestFrom(lastLocalHeartbeat$), map(([{ status }, lastLocalHeartbeat]) => {
        if (!lastLocalHeartbeat) {
            return status;
        }
        const lostHeartbeat = deviceHasLostHeartbeat(status, lastLocalHeartbeat);
        return lostHeartbeat
            ? Object.assign(Object.assign({}, status), { state: STATUS.OFFLINE }) : status;
    }), distinctUntilChanged((a, b) => isEqual(a, b)));
}
export function deviceHasLostHeartbeat(status, lastLocalHeartbeat) {
